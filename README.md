...
Aura is a real-time, privacy-conscious emotion intelligence platform. It captures live audio via WebRTC, runs high-speed local inference for immediate emotional context, and utilizes Google Gemini 3.1 Pro for deep multimodal analysis of full utterances (transcription + emotion judging in one call). The architecture is modular, separating the visually rich frontend client from the robust audio-processing Python backend.
...
┌───────────────────────────────┐   ┌───────────────────────────────────────────┐
│ Module 4: Fast-Path Inference │   │        Module 5: Google Gemini Cloud      │
│   Local ONNX SenseVoice-Small │   │ Unified Gemini 3.1 Pro (Multimodal)       │
│   (CPU) → 5 emotion classes   │   │ Transcription + Emotional Judging (~800ms)│
└───────────────────────────────┘   └───────────────────────────────────────────┘
                    │                               │
                    └───────────────┬───────────────┘
                                    ▼
...
- **API Keys**: You will need keys for [LiveKit Cloud](https://livekit.io/), [Supabase](https://supabase.com/), and [Google Gemini](https://aistudio.google.com/).
...
2. Fill in your credentials for LiveKit, Supabase, and Gemini.
...
- **Backend**: Python, FastAPI, asyncio, LiveKit Server SDK, Supabase Python SDK, Google GenAI SDK.
- **AI/ML Models**: Silero VAD, SenseVoice (via sherpa-onnx), Gemini 3.1 Pro (Multimodal).
`run.ps1`**: Root orchestrator for unified setup, development, and maintenance.
